...
async function incrementerCompteurs(patient) {
  try {
    ...
    // source fiable pour les entrées/sorties. Ici on ne compte que les actes cliniques,
    ...
    if (patient.crp_test) inc('nbActimCRP');
    if (patient.tdr_palu) inc('nbTdrPalu');
    if (patient.tdr_dengue) inc('nbTdrDengue');
    if (patient.quicktest) inc('nbTetanotop');
    if (patient.bu_fait) inc('nbBU');
    if (patient.bhcg_fait) inc('nbTGrossesse');
    ...
    prescriptions.filter(r=>r.fait).forEach(r => {
      ...
      if (t.includes('bilan sanguin') || t.includes('bio délocalisée')) inc('nbBilanSanguin');
      if (t.includes('ecbu')) inc('nbECBU');
      if (t.includes('hémoculture')) inc('nbHemocult');
      if (t.includes('coproculture')) inc('nbCoprocultures');
      ...
      if (t.includes('sonde urinaire')) inc('nbSondeUrinaire');
      if (t.includes('aérosol')) inc('nbAerosol');
      if (t.includes('meopa')) inc('nbMEOPA');
      if (t.includes('o2')) inc('nbOxygene');
      if (t.includes('tensiomètre')) inc('nbTensiometre');
      if (t.includes('drp')) inc('nbDRP');
      if (t.includes('pansement simple')) inc('nbPansementSimple');
      if (t.includes('pansement complexe')) inc('nbPansementComplexe');
      if (t.includes('lavage cae')) inc('nbLavageCAE');
      if (t.includes('vaccin')) {
        if (t.includes('covid')) inc('nbVaccinsCovid'); else inc('nbAutresVaccins');
      }
      if (t.includes('ablation abcès') || t.includes('ablation abces')) inc('nbAbces');
      ...
      if (t.includes('ducation')) inc('nbEducation'); // éducation / éducation asthme / éducation DRP
      if (t.includes('maternité')) inc('nbMaternite');
