              ...
              <div style={{display:'flex',gap:5,alignItems:'flex-start'}}>
                ...
                <div style={{minWidth:0,flex:1}}>
                  ...
                  <div style={{color:'#6b7280',fontSize:9,marginTop:1,display:'flex',alignItems:'center',gap:3}}>
                    ...
                    <span>{p.ddn&&(()=>{const[y,m,d]=p.ddn.split('-');return d&&m&&y?`${d}/${m}/${y.slice(-2)}`:p.ddn;})()}</span>
                  </div>
                </div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'4px 5px'}}>
                {[
                  {k:'fc',      v:p.fc,                                          u:'bpm',  icon:'🫀'},
                  {k:'tas',     v:p.tas&&p.tad?p.tas+'/'+p.tad:p.tas||'--',     u:'',     icon:'🩸'},
                  {k:'sat',     v:p.sat||'--',                                   u:'%',    icon:'💧'},
                  {k:'temp',    v:p.temp||'--',                                  u:'',     icon:'🌡️'},
                  {k:'dextro',  v:p.dextro||'--',                                u:'g/L',  icon:'🍬'},
                  {k:'hemocue', v:p.hemocue||'--',                               u:'g/dL', icon:'🔴'},
                ].map(({k,v,u,icon})=>{
                  ...
                  return(
                    <div key={k} style={{background:'rgba(255,255,255,0.7)',borderRadius:5,padding:'3px 5px',border:'0.5px solid rgba(0,0,0,0.06)',display:'flex',alignItems:'baseline',gap:3}}>
                      <span style={{fontSize:11}}>{icon}</span>
                      <span style={{fontSize:15,fontWeight:700,color:col?.color||'#374151',whiteSpace:'nowrap'}}>{v}</span>
                      {u&&<span style={{fontSize:8,color:'#9ca3af'}}>{u}</span>}
                    </div>
                  );
                })}
              </div>
